...
parser.add_argument('--creator', help="File with name of creator in it", required=True, type=str)
...
creator_file = open(args.creator, 'r')
creator_line = creator_file.read().splitlines()

print(len(creator_line))
base_line = "INSERT INTO public.magic_entity({}) VALUES\n"


def generateLine(k):
    data_line = [
        ('name', name_line[k].replace('\n', '')),
        ('entity_creator', creator_line[random.randint(0, args.number-1)].replace('\n', '')),
        ('power', str(50)),
        ('spell', '\'Fire\''),
    ]
    return data_line
